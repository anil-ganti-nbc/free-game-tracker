...
def test_prime_event_fields(payload: list[dict[str, Any]]) -> None:
    event = next(e for e in gamerpower.parse_giveaways(payload) if e.title == "Prime Freebie")
...
def test_missing_worth_and_end_reduce_confidence(payload: list[dict[str, Any]]) -> None:
    event = next(e for e in gamerpower.parse_giveaways(payload) if e.title == "Humble Giveaway")
