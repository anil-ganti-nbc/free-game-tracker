...
def test_sync_deals_dedupes_repeated_appid_in_one_run(db: None) -> None:
    """Steam's specials feed can list the same appid twice in one trawl.

    Without deduping, both rows are "new" in the same insert batch and the
    second violates steam_deals' appid UNIQUE constraint (IntegrityError),
    which previously bubbled up as an unhandled 500 from /api/run.
    """
    database.sync_deals([_deal(7), _deal(7)])
    assert {d.appid for d in database.load_deals()} == {7}
