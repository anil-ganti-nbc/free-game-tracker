...
def test_subscription_events_pass_gate() -> None:
    from newsroom.models import Category

    event = _event(score=100, price=0.0)
    event.category = Category.SUBSCRIPTION
    assert passes_quality_gate(event, min_confidence=100, require_known_price=True) is True
