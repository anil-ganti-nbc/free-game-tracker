...
def test_failing_source_is_isolated(temp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """One source raising an unexpected error must not lose the others."""

    def _boom() -> list[NewsEvent]:
        raise RuntimeError("parser blew up")

    monkeypatch.setattr(cli, "_SOURCES", {"good": lambda: [_event("a")], "bad": _boom})
    events, _ = cli._fetch_all_sources([])
...
def test_source_filter_selects_subset(temp_env: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        cli, "_SOURCES", {"one": lambda: [_event("a")], "two": lambda: [_event("b")]}
    )
    events, _ = cli._fetch_all_sources(["two"])
