...
def collect(source: str, *, get: reddit.GetPage | None = None, persist: bool = True,
            code_revision: str = "UNKNOWN") -> dict[str, Any]:
    ...
    with db.session_scope() as session:
        prior = session.scalar(select(db.DiscoveryRunRow.id).where(
            db.DiscoveryRunRow.source == source, db.DiscoveryRunRow.status == "ok").limit(1))
        baseline = prior is None
        run = db.DiscoveryRunRow(source=source, started_at=now, baseline=baseline,
    ...
    try:
        ...
        with db.session_scope() as session:
            for post in posts:
                ...
                evidence = post.evidence() | {"run_id": run_id, "source_id": source,
                            "code_revision": code_revision, "collected_at": now.isoformat(),
                            "classification_policy": "fgt-reddit-discovery-v1"}
                if row is None:
                    row = db.DiscoveryObservationRow(
                        key=key, source=source, external_id=post.external_id,
                        title=post.title, url=post.permalink, classification=reddit.classify(post),
                        baseline=baseline, first_seen=now, last_seen=now, evidence=[evidence])
                    session.add(row)
                    new += 1
                else:
                    row.last_seen = now
            ...
            saved_run.evidence = {"observed": len(posts), "new_observations": new,
                            "collection_health": "ok", "persistence_health": "ok",
                            "delivery": "blocked", "code_revision": code_revision}
        db.record_source_result(source, ok=True, count=len(posts))
        return {"observed": len(posts), "new_observations": new,
                "baseline": baseline, "delivery": "blocked"}
        ...
        with db.session_scope() as session:
            ...
            saved_run.finished_at = datetime.now(UTC)
            saved_run.evidence = {"error": str(exc), "delivery": "blocked", "code_revision": code_revision}
