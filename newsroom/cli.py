...
from newsroom.sources import epic, gamerpower, gog, steam, steam_breakouts, steam_deals
...
def _fetch_all_sources(selected: list[str] | None) -> list[NewsEvent]:
...
def _stale_sources(health: list[SourceHealth], stale_hours: int) -> list[str]:
    ...
    return [
        h.source
        for h in health
        if h.last_success_at is None or h.last_success_at < cutoff
    ]
...
def _execute_run(
    ...
    Split out from the ``run`` command so the pipeline can be tested end-to-end
    without any network fetch. The previous state is loaded *before* the current
    events are persisted, so the comparison sees a true before/after. When
    ``persist`` is False (a dry run) the database is left untouched.

    Everything detected is stored, but only detections that clear the quality
    gate are surfaced in the returned diff and the reports.
    """
...
def run_pipeline(
    ...
    This is the single orchestration path shared by the ``run`` CLI command and
    the web dashboard's "Run now" action: fetch every selected source (isolated),
    fetch Epic's upcoming heads-up, compare/store/report, then notify. Returns a
    small summary dict for the caller to present however it likes.
    """
    init_db()
    generated_at = datetime.now(UTC)

    current_events = _fetch_all_sources(selected)
    upcoming = _fetch_upcoming(selected)
    diff, markdown_path, json_path = _execute_run(
        current_events, generated_at, ending_soon_hours, persist=persist, upcoming=upcoming
    )
...
def identity_cmd() -> None:
    """Emit Stage 0.5 runtime identity JSON (Stage 1.2 portability)."""
    import json
    from newsroom.runtime_bridge import as_jsonable, get_identity
    print(json.dumps(as_jsonable(get_identity()), indent=2))


@app.command("health")
def health_cmd() -> None:
    """Emit Stage 0.5 health JSON without running collectors (Stage 1.2)."""
    import json
    from newsroom.runtime_bridge import as_jsonable, get_health
    print(json.dumps(as_jsonable(get_health()), indent=2))
...
def version() -> None:
    ...
    import json
    from newsroom.runtime_bridge import get_version_info
    # Human line + machine JSON for portability consumers
