...
def load_discovery_observations(*, include_baseline: bool = False) -> list[dict[str, Any]]:
    ...
    if not include_baseline:
        query = query.where(DiscoveryObservationRow.baseline.is_(False),
            DiscoveryObservationRow.classification.in_(["giveaway_claim_unverified", "game_rumour_unverified"]))
    query = query.order_by(DiscoveryObservationRow.first_seen.desc()).limit(200)
    with session_scope() as session:
        rows: list[dict[str, Any]] = [{"source": r.source, "external_id": r.external_id, "title": r.title,
                 "url": r.url, "classification": r.classification,
                 "baseline": r.baseline, "first_seen": r.first_seen.isoformat(),
                 "novelty": "unconfirmed", "delivery": "blocked", "evidence": r.evidence}
        ...
        for row in rows:
            row["related_observation_ids"] = sorted({
                identity for url in row["evidence"][-1].get("related_url_keys", [])
                for identity in related[url] if identity != row["external_id"]})
        return rows
