...
def sync_new_releases(releases: list[NewRelease]) -> None:
    ...
    with session_scope() as session:
        existing = {
            row.appid: row for row in session.scalars(select(NewReleaseRow)).all()
        }
...
def sync_deals(deals: list[SteamDeal]) -> None:
    ...
    with session_scope() as session:
        existing = {
            row.appid: row for row in session.scalars(select(SteamDealRow)).all()
        }
        for appid, row in existing.items():
            if appid not in current_ids:
                session.delete(row)
...
def sync_events(events: list[NewsEvent]) -> None:
    """Reconcile the stored snapshot with this run's events.

    The table always reflects the *latest* run's set of currently-free games:

    * offers seen for the first time are inserted,
    * offers seen before are refreshed in place (keeping ``discovered_at``),
    * offers that are no longer present are deleted.

    Pruning vanished offers is what keeps the comparison honest — otherwise
    every past giveaway would be re-reported as "expired" on every run. Called
    once per run, after the comparison has already read the previous state.
    """
    current_keys = {event.event_key for event in events}
    with session_scope() as session:
        existing_by_key = {
            row.event_key: row
            for row in session.scalars(select(NewsEventRow)).all()
        }
