...
def fetch_text(
    ...
    *,
    ...
    """GET ``url`` and return raw text string, retrying transient failures."""
    owns_client = client is None
    client = client or httpx.Client(headers=headers or DEFAULT_HEADERS)
    try:
        return _get_text_with_retries(client, url)
    finally:
        if owns_client:
            client.close()
...
def _get_text_with_retries(client: httpx.Client, url: str) -> str:
    """GET a URL and return raw text, retrying up to the configured limit."""
    last_error: Exception | None = None
    for attempt in range(settings.http_max_retries + 1):
        try:
            response = client.get(url, timeout=settings.http_timeout_seconds, follow_redirects=True)
            response.raise_for_status()
