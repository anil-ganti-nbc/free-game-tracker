...
class Settings(BaseSettings):
    ...
    enable_reddit_fgf_delivery: bool = False
    ...
    deal_scan_pages: int = Field(
        default=5,
        ge=1,
        le=20,
        description=(
            "How many pages (100 candidates each) of Steam's full discounted-specials "
            "listing to scan per run. Bounded on purpose: Steam's storefront "
            "'featuredcategories' specials carousel only ever exposes 10 curated "
            "picks, which misses the vast majority of real discounts, but scanning "
            "the full listing (thousands of items) every run risks rate limiting. "
            "Raise cautiously if deals are still being missed."
        ),
    )
