...
class NewsEvent(BaseModel):
    ...
    developer: Optional[str] = Field(
        default=None,
        description="Developer/publisher name (for game promotions)"
    )
    publisher: Optional[str] = Field(
        default=None,
        description="Publisher name (for game promotions)"
    )
    promotion_type: Optional[PromotionType] = Field(
        default=None,
        description="Type of promotion (for game promotions)"
    )
    original_price: Optional[float] = Field(
        default=None,
        description="Original price in USD (for promotions)"
    )
    current_price: Optional[float] = Field(
        default=None,
        description="Current price in USD (for promotions)"
    )
    
    # Timing
    promotion_start: Optional[datetime] = Field(
        default=None,
        description="When the event/promotion starts"
    )
    promotion_end: Optional[datetime] = Field(
        default=None,
        description="When the event/promotion ends"
    )
    
    # Tracking
    discovered_at: datetime = Field(
        default_factory=datetime.utcnow,
        description="When we first discovered this event"
    )
    last_seen: datetime = Field(
        default_factory=datetime.utcnow,
        description="Last time we verified this event exists"
    )
    
    # Confidence
    confidence_score: Confidence = Field(
        default=Confidence(0.5),
        description="How certain we are this event is real and significant (0.0-1.0)"
    )
    
    # Extensibility
    metadata: dict[str, Any] = Field(
        default_factory=dict,
        description="Source-specific metadata (arbitrary key-value pairs)"
    )
    ...
    def price_reduction_percent(self) -> Optional[float]:
